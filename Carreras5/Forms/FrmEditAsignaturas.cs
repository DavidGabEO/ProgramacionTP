...
namespace Carreras5
{
    public partial class FrmEditAsignaturas : Form
    {
        ...
        {
            if (ExisteAsignatura(txtAsignatura.Text))
            {
                MessageBox.Show("La asignatura ya esta registrada", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        {
            if (dgvAsignaturas.CurrentCell.ColumnIndex == 2)
            {
                int AsignaturaABorrar = ((int)dgvAsignaturas.CurrentRow.Cells["colId"].Value);
                app.BorrarAsignaturaYDetalles(AsignaturaABorrar);
        ...
        private bool ExisteAsignatura(string text)
        {
            ...
            {
                if (fila.Cells["colNombre"].Value.Equals(text))
                    return true;
            }
            return false;
        }
