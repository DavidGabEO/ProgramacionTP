...
namespace CarrerasBackend.datos
{
    class CarreraDao : ICarreraDao
    {
        ...
        {
            return HelperDao.GetInstance().ConseguirIdCarrera(nombre, "SP_CONSEGUIR_ID_CARRERA");
        }

        public bool SaveSoloDetalle(DetalleCarrera detalle, int idCarrera, int idAsignatura)
        {
            return HelperDao.GetInstance().InsertarSoloDetalle(detalle, "SP_INSERTAR_DETALLESCARRERAS", idCarrera, idAsignatura);
        }

        public bool DeleteUnDetalle(int idAsignatura)
        {
            return HelperDao.GetInstance().BorrarUnDetalle(idAsignatura, "SP_BORRAR_DETALLE_ASIGNATURA");
        }

        public bool DeleteAsignaturaYDetalle(int idAsignatura)
        {
            return HelperDao.GetInstance().BorrarAsignaturasYDetalles(idAsignatura, "SP_BORRAR_ASIGNATURA_Y_DETALLES");
        }

        public bool DeleteCarreraYDetalles(int idCarrera)
        {
            return HelperDao.GetInstance().BorrarCarreraYDetalles(idCarrera, "SP_BORRAR_CARRERA_Y_DETALLES");
        }
    }
}
